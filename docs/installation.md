...
*Phương án này chạy trực tiếp NodeJS, PostgreSQL, Redis, Go trên Windows để tối ưu tốc độ phản hồi khi code. Docker chỉ được cài đặt để làm môi trường Sandbox cho Worker chấm bài.*
...
5. **Redis cho Windows**: 
   * Tải và cài đặt bản Redis dành cho Windows (ví dụ: [Memurai](https://www.memurai.com/) hoặc bản port của MSOpenTech từ Github).
   * Đảm bảo dịch vụ Redis chạy ở cổng mặc định `6379`.
...
1. **API Gateway (`apps/api`)**:
   ...
   APP_REDIS_CONN_STRING=redis://127.0.0.1:6379
...
## <img src="https://images.weserv.nl/?url=https://api.iconify.design/solar/box-bold-duotone.svg?color=%25230070f3&output=png&w=28&h=28" width="28" height="28" align="center" /> PHƯƠNG ÁN 2: Sử dụng Docker Compose (All-in-One cho DB/Redis)
*Phương án này giúp bạn không cần cài đặt thủ công PostgreSQL và Redis trực tiếp trên máy Windows, mọi thứ được đóng gói qua Docker.*
...
### <img src="https://images.weserv.nl/?url=https://api.iconify.design/solar/tuning-bold-duotone.svg?color=%25230070f3&output=png&w=20&h=20" width="20" height="20" align="center" /> Bước 2: Khởi động DB và Redis bằng Docker
Tại thư mục gốc của dự án (`truesubmit/`), nơi có file `docker-compose.yml`, mở PowerShell và chạy:
```powershell
docker compose up -d
```
Lệnh này sẽ tự động tải và khởi chạy PostgreSQL cùng Redis trong nền. Bạn có thể kiểm tra trạng thái bằng lệnh `docker compose ps`.
...
1. Copy cấu hình `.env` cho `apps/api` và `apps/worker` y hệt như Phương án 1 (do cổng DB/Redis của container được map trực tiếp ra `localhost`).
2. Thực hiện cài đặt dependencies và chạy các dự án:
   ...
   * **Terminal 2 (Frontend)**: `cd apps/web && pnpm run dev`
