...
- **Quy trình xử lý cực nhanh (< 15ms)**:
  ...
  3. Đẩy một Job chứa thông tin `{ submissionId, language, code, problemId }` vào **Redis Queue** (dạng List/Stream) thông qua lệnh `LPUSH`.
  4. Trả ngay mã `201 Created` kèm theo `submissionId` cho frontend.
